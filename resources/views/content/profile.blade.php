    ...
    <div class="profile-image-up">
        ...
        <div class="button-nav">
            ...
            $isFollow = $follow->where('follower_id', $user->id)
                            ...
                            ->isNotEmpty();
            @endphp
            <button class="radius follow-btn" type="button" data-follower-id="{{ $user->id }}">{{ $isFollow ? 'フォロー解除' : 'フォローする'}}</button>
        </div>
        <div class="column">
            <p>{{ $follow_count }}フォロー中</p>
            <p class="follower-count">{{ $follower_count }}フォロワー</p>
        </div>
